...
export default function VerificationScreen() {
  ...
  const handleContinue = () => {
    if (isComplete) {
      // Navigate to name screen after verification using replace to prevent going back
      // This replaces verification in history, so user can't go back to phone/verification screens
    ...
    >
      ...
      <div className="flex-1 px-4 sm:px-6 overflow-y-auto" style={{ paddingTop: '60px'  }}>
        ...
        <p className="text-xs sm:text-sm text-[#1A1A1A] mb-4 sm:mb-6" style={{ fontFamily: "Sweet Sans Pro, -apple-system, sans-serif" }}>
          Enter the 6-digit code sent to you at 984561256
        </p>
        ...
        <button 
          ...
          style={{ 
            backgroundColor: isComplete ? '#1A9952' : '#A5D6A7',
        ...
        <div className="flex justify-center items-center gap-1 pb-4">
          ...
          <button className="text-xs sm:text-sm text-[#1A1A1A] font-medium" style={{ fontFamily: "Sweet Sans Pro, -apple-system, sans-serif" }}>
            Resend OTP
          </button>
        </div>
      </div>
