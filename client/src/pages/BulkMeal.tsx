...
import { ArrowLeft, Building2, Users, Calendar, Mail, Phone, MapPin, ShoppingCart, UtensilsCrossed, Package, Truck, Clock, X, ChevronDown, Search, Mic, ArrowUpDown, SlidersHorizontal, Star, Utensils, LayoutGrid, Leaf, Drumstick, Egg, Sparkles, CheckCircle } from "lucide-react";
...
export default function BulkMeals({ onNavigate }: BulkMealsProps = {}) {
  ...
  const handleAddToCart = (item: { id: number; name: string; price: number; quantity?: number; image?: string }) => {
    ...
    addToCart("bulk-meals", {
      ...
      price: item.price,
      quantity,
      isSixtyMin: !!onNavigate && localStorage.getItem(SIXTY_MIN_ORDER_FLAG) === "true",
      image: item.image,
  ...
  const filteredAndSortedDishes = useMemo(() => {
    ...
    return searchResults
      .filter(dish => {
        ...
        // AND the user is actually in a 60-min delivery area
        if (onNavigate && localStorage.getItem(SIXTY_MIN_ORDER_FLAG) === "true") {
  ...
  }, [dishes, debouncedSearchQuery, fuse, selectedDishType, dietaryMode, priceRange, sortOption, selectedCategory, priorityCategoryId, onNavigate, festiveFilter]);
  ...
  if (isInitialLoading) {
    return (
      <div className="min-h-screen pb-24 relative bg-transparent">
        {/* Header with Back Button - Hidden since unified header has hamburger menu */}
        ...
        <div className="relative z-10 px-4 pt-4 pb-6">
          ...
          <div className="grid grid-cols-4 gap-2 lg:gap-4 mb-8 max-w-2xl lg:max-w-4xl mx-auto">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="aspect-square rounded-[10px] bg-gray-200 animate-pulse"
              />
            ))}
          </div>
        </div>
  ...
  return (
    <PageWithLoader>
      {/* Hero Background - Mobile only */}
          ...
          <div className="relative z-10 px-4 pt-4 pb-6">
            ...
            <div className={onNavigate ? "grid grid-cols-2 gap-3 max-w-xs mx-auto" : "grid grid-cols-4 gap-2 lg:gap-4 max-w-2xl lg:max-w-4xl mx-auto"}>
              ...
              {!onNavigate && (
                <>
                  <button
                    onClick={() => { setSelectedService("snack-box"); navigate("/snack-box"); }}
        ...
        >
          {/* Search Bar - Clicking Opens Overlay */}
          <div
          ...
          >
            <div className="relative">
              ...
              <div
                className="w-full pl-12 pr-4 py-2.5 bg-white text-sm text-gray-500"
                style={{
                  fontFamily: "Sweet Sans Pro",
                  borderRadius: "10px",
        ...
        <div className="relative z-10 px-4 pt-2">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-0 flex-1 w-full max-w-full">
              ...
              <aside
                className="w-20 md:w-24 border-r bg-card/50 backdrop-blur-sm flex-shrink-0 sticky self-start"
                style={{
                  top: isMd ? '312px' : '264px',
                  maxHeight: isMd ? 'calc(100vh - 312px)' : 'calc(100vh - 264px)',
                  overflowY: 'auto'
                }}
              >
